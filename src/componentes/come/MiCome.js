...
class MiCome {
	constructor() {
		...
		let bSalirCome = document.createElement('button');
		bSalirCome.type = 'button';
		bSalirCome.className = "bSalirCome";
		bSalirCome.id = 'bSalirCome';
		bSalirCome.onclick = salirCome;
		bSalirCome.textContent = "Salir";
		document.body.appendChild(bSalirCome);
		...
		let miformulariocontrol = document.createElement('input')
		...
		function botonmovil() {
			...
			let tecla1 = 1000; let tecla2 = 1000; let son = 1; comenzar = setInterval(iniciojuego, 200)
		...
		function mueveraton(e) {
			if (vraton == 1) { return }
			e.preventDefault();
			let we = e.targetTouches.length
			let touch = e.targetTouches[we - 1]
		...
		function controlraton(e) {
			...
			let a3y = 78
			let a3x = 78
			let t0 = (a1x - a3x) * (a2y - a3y) - (a1y - a3y) * (a2x - a3x)
			let t1 = (a1x - rx) * (a2y - ry) - (a1y - ry) * (a2x - rx)
			let t2 = (rx - a3x) * (a2y - a3y) - (ry - a3y) * (a2x - a3x)
			let t3 = (a1x - a3x) * (ry - a3y) - (a1y - a3y) * (rx - a3x)
		...
		let etiqueta = document.createElement("text")
		...
		let etinivel = document.createElement("text")
		...
		let cronogeneral
		let puntuacion = 0
		let matrix = new Array(22);
		for (let i = 0; i < 22; i++) { matrix[i] = new Array(22); }
		let accion
		let espera
		let kcartel1
		let rx
		...
		let controlflas
		let controlrota
		let posix = new Array(4)
		let posiy = new Array(4)
		let labex = new Array(4)
		let labey = new Array(4)
		let velox = new Array(4)
		let veloy = new Array(4)
		let dire = new Array(4)
		let pderecha = new Array(4)
		let pabajo = new Array(4)
		let pizquierda = new Array(4)
		let parriba = new Array(4)
		let r = new Array(4)
		let situ = new Array(4)
		let peli
		let cuentacome
		...
		let nfruta
		let cuentafruta
		let fruy = new Array(4)
		let frux = new Array(4)
		let afruy = new Array(4)
		let afrux = new Array(4)
		let tfruy = new Array(4)
		let tfrux = new Array(4)
		let cf = new Array(4)
		//VARIABLES FANTASMA
		let nfan
		let controlflasfan = new Array(4)
		let ale
		let z = new Array(4)
		let n
		let contadorhuir
		...
		function cargarvariables() {
			...
			for (let i = 1; i <= 4; i++) { fancanvas[i].style.visibility = "hidden"; }
			...
			for (let i = 1; i <= contadorvida; i++) { vida[i].style.visibility = "visible"; }
		...
		let cargapagina = setTimeout(cargada, 1500)
		...
		function juegonuevo() {
			...
			for (let i = 1; i <= 4; i++) { cartel[i].style.visibility = "hidden"; }
			cartelfruta.style.visibility = "hidden"
			cartelvida.style.visibility = "hidden"
			etinivel.style.visibility = "hidden"
			for (let i = 0; i < 22; i++) {
				for (let j = 0; j < 22; j++) { punto[i][j].style.visibility = "hidden" }
			}
			...
			for (let i = 1; i <= 4; i++) { posix[i] = 220 + (i - 1) * 25 + i * 10; dire[i] = 3; situ[i] = 0 }
			nivel = 1
			contadorvida = 3
			cuentacome = 0
			velocidadfan = 4
			comecanvas.style.visibility = "visible"
			for (let i = 1; i <= 4; i++) { fancanvas[i].style.visibility = "visible" }
			...
			switch (accion) {
				case 0:
					...
					if (posix[0] <= -300) {
						accion = 1
						comecanvas.style.transform = "rotate(0deg)"
						for (let i = 1; i <= 4; i++) { velox[i] = 1; dire[i] = 1; situ[i] = 2 }
					}
					break;
				case 1:
					for (let i = 1; i <= 4; i++) {
					...
					posix[0] = posix[0] + velox[0]
					ponecome()
					if (posix[0] <= -370) { for (let i = 1; i <= 4; i++) { velox[i] = 0.6 }; velox[0] = 1 }
					for (let i = 1; i <= 4; i++) {
					...
					if (posix[4] >= 650) {
						...
						comecanvas.style.transform = "rotate(180deg)";
						for (let i = 1; i <= 4; i++) { velox[i] = -1; situ[i] = 1; dire[i] = 3 }
						for (let i = 0; i <= 4; i++) { fruy[i] = 0; frux[i] = 0; afruy[i] = 0; afrux[i] = 0; tfruy[i] = 0; tfrux[i] = 0; cf[i] = 0 }
						fruy[0] = -300
						frux[0] = 10
						afruy[0] = 1
		...
		function movicartelera() {
			...
			cartelera.style.top = fruy[0] + "px"
			cartelera.style.left = frux[0] + "px"
			if (fruy[0] >= 87.5) { accion = 3; tfruy[0] = 0; espera = setTimeout(espera1, 5000) }
		}
		...
		function iniciojuego() {
			...
			ponecome()
			comecanvas.style.visibility = "visible"
			for (let i = 1; i <= 4; i++) { ponefan(i); fancanvas[i].style.visibility = "visible" }
			for (let i = 1; i <= 6; i++) {
				fruta[i].style.visibility = "hidden";
				fruta[i].style.top = "300px";
				fruta[i].style.left = "35px"
			...
			cartelfruta.style.visibility = "hidden"
			let jugar = setTimeout(iniciarcronometros, 5000);
		}
		...
		function miracuentacome() {
			if (cuentacome == 185) {
				...
				sonido[7].play()
				let finnivel = setTimeout(nuevonivel, 5000);
				return
			}
		...
		function irpunto(a, p1, p2, v) {
			...
			let w = pderecha[a] * 8 + pabajo[a] * 4 + pizquierda[a] * 2 + parriba[a]
		...
		function movicartelfan() {
			for (let a = 1; a <= contadormuertefan; a++) {
				if (cf[a] == 1) {
					tfruy[a] = tfruy[a] + 0.02
		...
		function muertecome(a) {
			...
			if (Math.abs(posiy[0] - posiy[a]) <= 10 && Math.abs(posix[0] - posix[a]) <= 10) {
				...
				for (let i = 1; i <= 4; i++) { velox[i] = 0; veloy[i] = 0 }
				clearInterval(cronoiniciocome)
				clearInterval(cronoiniciofan)
				verpelimuerte()
		...
		function caja(a) {
			...
			if (situ[a] == 5) { return }
			let c = Math.round(Math.random() * (100 - 0) + parseInt(0));
			if (c == a) {
		...
		function llenar() {
			...
			let ay = 5
			let p = 1
			for (let i = 0; i < 22; i++) { for (let j = 0; j <= 18; j++) { matrix[i][j] = f[i].charAt(j) } }
			...
			if (kk == 1) { return }
			for (let i = 1; i <= 6; i++) { miformulario.appendChild(fruta[i]); }
			for (let i = 1; i <= 4; i++) { miformulario.appendChild(fancanvas[i]); }
			miformulario.appendChild(comecanvas);
			for (let i = 1; i <= 10; i++) { miformulario.appendChild(muerte[i]); }
			miformulario.appendChild(ready);
			for (let i = 1; i <= 4; i++) { miformulario.appendChild(cartel[i]); }
			for (let i = 1; i <= 3; i++) { miformulario.appendChild(vida[i]); }
		...
		function eliminar() {
			document.removeEventListener('onclick', repitesonido);
			son=1;
			clearInterval(comenzar);
			clearInterval(cronocome);
			clearInterval(cronofan);
			clearInterval(movipanta);
			clearInterval(kcartel1);
			clearInterval(cronoiniciocome);
			clearInterval(cronoiniciofan);
			clearInterval(cronofruta);
			clearInterval(kcartel);
			clearInterval(pelimuerte);
			clearInterval(cargapagina);			
			for (let i = 1; i <= 8; i++) {
				sonido[i].pause()
